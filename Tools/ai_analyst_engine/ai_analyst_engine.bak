...
def generate_prompt(skill_path, evidence_path, output_dir, silent=False):
    """Combines the skill and evidence into a single prompt file."""
    try:
        # Read the Skill
    ...
    # Construct the fused prompt
    timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
    skill_name = os.path.basename(skill_path).replace('_SKILL.md', '')
    evidence_name = os.path.basename(evidence_path)
    
    prompt = f"""You are acting as an expert SOC Analyst. Please apply the specialized methodology defined in the [ANALYST SKILL] section below to thoroughly analyze the data provided in the [RAW EVIDENCE] section.
...
==================================================
Please generate your analysis now based strictly on the evidence and methodology above.
"""
    ...
    # Also maintain a stable alias for automation pipelines
    ...
    if not silent:
        print(f"\n{Colors.GREEN}[+] AI Analysis Prompt successfully generated!{Colors.ENDC}")
    ...
    if not args.silent:
        ...
        try:
            ...
            if 0 <= skill_idx < len(skills):
                ...
                generate_prompt(skill_path, evidence_path, output_dir, args.silent)
            else:
