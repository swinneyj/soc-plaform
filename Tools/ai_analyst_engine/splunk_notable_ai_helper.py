...
def resolve_detection_science(platform_root: str, explicit_path: str = "") -> str:
    """Locates detection science / supportive rule logic if available.

    Checks:
    1) Explicit path passed via CLI / parameter.
    2) supportive_rules.json in platform root.
    3) Detection science profiles in Data/AI_Knowledge_Base.
    """
    if explicit_path:
        candidate = explicit_path.strip('"').strip("'")
        if not os.path.isabs(candidate):
            candidate = os.path.join(platform_root, candidate)
        if os.path.exists(candidate):
            return candidate

    # Auto-discovery fallback: supportive_rules.json in repo root
    default_supportive = os.path.join(platform_root, 'supportive_rules.json')
    if os.path.exists(default_supportive):
        return default_supportive
...
def main() -> None:
    ...
    parser.add_argument('--detection-science', type=str, required=False, default="",
                        help="Path to detection science / supportive rules (JSON/MD)")
    ...
    # Resolve detection science context
    detection_science_path = resolve_detection_science(platform_root, args.detection_science)
    ...
    if not args.silent:
        ...
        if detection_science_path:
            print(f"{Colors.GREEN}[+] Detection Science:{Colors.ENDC} {os.path.basename(detection_science_path)}")
        else:
            print(f"{Colors.WARNING}[!] Detection Science:{Colors.ENDC} None (using general methodology)")

    # Generate the fused prompt file (passing detection science)
    try:
        generate_prompt(
            skill_path, 
            evidence_path, 
            output_dir, 
            detection_science_path=detection_science_path, 
            silent=args.silent
        )
