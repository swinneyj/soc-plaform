...
def import_rules_from_json(
    ...
    replace_all_supportive: bool = False,
) -> dict:
    """Import rules from JSON file into database.

    Supportive query policy:
      - Default (merge): insert new (rule_id, title); update SPL/description on
        title match; leave DB-only rows alone.
      - replace_rule_ids: for those rule_id values, delete ALL supportive rows
        for the rule, then insert the JSON set (JSON is full playbook for that rule).
      - replace_all_supportive: same wipe+insert for every rule present in the JSON.
    """
    ...
    try:
        ...
        replace_set = {str(r).strip() for r in (replace_rule_ids or []) if str(r).strip()}
        if replace_all_supportive:
            replace_set |= {str(r.get("rule_id") or "").strip() for r in rules if r.get("rule_id")}
        ...
        for rule_data in rules:
            try:
                ...
                do_replace = rule_id in replace_set

                # Optional: import any supportive queries tied to this rule.
                #
                # Merge policy (per rule, per title) — default:
                #   - If JSON has a new (rule_id, title): insert.
                #   - If JSON has same (rule_id, title): update description/SPL.
                #   - If DB has extra rows not in JSON: keep them (do not delete).
                #
                # Replace policy (--replace-rule / --replace-all-supportive):
                #   - Delete all supportive rows for this rule_id, then insert JSON set.
                if do_replace:
                    deleted = (
                        db.query(SupportiveQuery)
                        .filter(SupportiveQuery.rule_id == rule_id)
                        .delete(synchronize_session=False)
                    )
                    supportive_deleted += int(deleted or 0)
        ...
        if not silent:
            ...
            print(
                f"    Supportive Queries - Imported: {supportive_imported} | "
                f"Updated: {supportive_updated} | Skipped: {supportive_skipped} | "
                f"Deleted(replace): {supportive_deleted}"
            )
            if replace_set and not silent:
                print(f"    Replace mode rule_ids: {', '.join(sorted(replace_set))}")
            if errors:
        ...
        return {
            ...
            "supportive_deleted": supportive_deleted,
            "errors": errors
        }
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Import Splunk ES correlation rules and supportive queries"
    )
    ...
    parser.add_argument(
        ...
        metavar='RULE_ID',
        help=(
            'Wipe all supportive queries for RULE_ID then insert the JSON set for that rule. '
            'Repeatable. Example: --replace-rule linux_ssh_key_creation'
        ),
    )
    parser.add_argument(
        '--replace-all-supportive',
        action='store_true',
        help='For every rule in the JSON, wipe DB supportive rows then insert the JSON set.',
    )
    
    args = parser.parse_args()
    
    if args.import_file:
        result = import_rules_from_json(
            args.import_file,
            args.silent,
            replace_rule_ids=args.replace_rule,
            replace_all_supportive=args.replace_all_supportive,
        )
