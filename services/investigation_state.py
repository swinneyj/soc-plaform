...
def _summarize_evidence_observation(raw_result: Dict[str, Any]) -> str:
    ...
    The evidence ledger/timeline is the durable record of what the analyst
    actually saw, so avoid over-aggressive truncation that cuts off
    multi-line tables or key context mid-sentence. A generous upper bound
    is kept only to prevent pathological payloads from overwhelming the UI.
    """
    ...
    if _is_substantive_evidence_value(analyst_summary):
        return analyst_summary[:MAX_EVIDENCE_SUMMARY_CHARS]
    ...
    if _is_substantive_evidence_value(result_text):
        return result_text[:MAX_EVIDENCE_SUMMARY_CHARS]
