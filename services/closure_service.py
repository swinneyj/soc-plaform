...
def generate_structured_closure_note(
    ...
    rule_name = rule.rule_name if rule else (case.rule_name or "Security Detection")
    ...
    note_sections = [
        f"# INCIDENT CLOSURE REPORT: {case_id}",
        f"**Date:** {date_str} | **Analyst:** analyst.swinney | **Final Verdict:** {formatted_disposition} ({confidence_pct} confidence){NL}",
        "## 1. Executive Summary",
        f"- **Correlation Rule:** {rule_name} (`{target_rule_id or 'unknown'}`)",
        f"- **Incident Summary:** {case.analysis_summary or 'No initial summary recorded.'}",
        f"- **Working Hypothesis:** {inv_state.get('current_hypothesis') or 'Hypothesis resolved through evidence collection.'}{NL}",
        "## 2. Investigation Closure Assessment",
        f"- **Closure Readiness:** {'Verified Ready' if readiness['is_ready'] else 'Overridden with Active Blockers'}",
        f"- **Loop Status:** {readiness['status'].replace('_', ' ').title()}",
    ]

    if readiness["blockers"]:
        note_sections.append("- **Documented Override Blockers:**")
        for b in readiness["blockers"]:
            note_sections.append(f"  - {b}")

    note_sections.extend([
        f"{NL}## 3. Evidence Ledger",
        format_evidence_ledger(evidence_rows),
        "## 4. Analysis Iteration Audit Trail",
        format_iteration_audit_trail(analyses),
    ])

    if analyst_notes:
        note_sections.extend([
            "## 5. Analyst Observations & Context",
            analyst_notes.strip() + NL,
        ])

    remediation = case.remediation_steps or "Review accounts, hosts, and update monitoring baseline as required."
    note_sections.extend([
        "## 6. Remediation & Follow-Up Actions",
        remediation.strip(),
        f"{NL}---",
        f"_Generated by SOC Platform v1.1.0 • Incident {case_id} officially resolved._",
    ])

    generated_note = NL.join(note_sections)
