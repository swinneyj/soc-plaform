"""Migrate SOC Platform data from a source database to PostgreSQL.

This utility is kept for one-time migration from legacy databases
(for example, an old SQLite backup) into the current PostgreSQL
runtime. It is not used by the running platform itself.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Migrate SOC Platform data from a source DB to PostgreSQL")
    parser.add_argument(
        "--source-url",
        default=os.environ.get("SOURCE_DATABASE_URL"),
        help="SQLAlchemy URL for the source database. For example, a preserved SQLite backup.",
    )
    parser.add_argument(
        "--target-url",
