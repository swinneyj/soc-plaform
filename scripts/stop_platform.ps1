...
# Always prefer the local .env settings so export_shared_logic_from_db.py
# and export_db_dump_to_share.ps1 connect with the same credentials as the
# running app. This intentionally overwrites any stale DATABASE_URL in the
# current environment.
...
if (Test-Path $envFile) {
    ...
    # When DATABASE_URL is not explicitly set, fall back to the same
    # password-bearing URL shape that start_platform.ps1 uses so that
    # exports work even on machines that require a password.
    if (-not $env:DATABASE_URL) {
        $databaseUser = "soc_platform"
        $databaseName = "soc_platform"
        $databasePassword = ""

        if ($envContent -match '(?m)^POSTGRES_USER\s*=\s*(.+?)\s*$') {
            $databaseUser = $Matches[1].Trim().Trim('"').Trim("'")
        }
        if ($envContent -match '(?m)^POSTGRES_DB\s*=\s*(.+?)\s*$') {
            $databaseName = $Matches[1].Trim().Trim('"').Trim("'")
        }
        if ($envContent -match '(?m)^POSTGRES_PASSWORD\s*=\s*(.+?)\s*$') {
            $databasePassword = $Matches[1].Trim().Trim('"').Trim("'")
        }
...
if (-not $env:DATABASE_URL) {
    # Fallback: construct a URL without a password for environments that
    # do not use password auth on the local Postgres instance.
