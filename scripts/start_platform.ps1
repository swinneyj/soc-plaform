...
if (-not $env:POSTGRES_PASSWORD) {
    ...
    if (Test-Path $envPath) {
        ...
        if ($envContent -match '(?m)^POSTGRES_PASSWORD\s*=\s*(.+?)\s*$') {
            ...
            Write-Host "[*] Resolved POSTGRES_PASSWORD from .env" -ForegroundColor DarkGray
        }
    }
}
if (-not $env:POSTGRES_PASSWORD) {
    Write-Error "POSTGRES_PASSWORD is required. Set it in .env or as an environment variable before starting."
    exit 1
}
...
if ($OpenBrowser) {
    # Prefer the modular UI shell (index.modular.html), which matches the
    # Linux testing path and has the full tab wiring for AI Analysis, etc.
    Start-Process "http://127.0.0.1:$ApiPort/index.modular.html"
}
